...
var noFill=nofill;var noStroke=nostroke;var strokeWidth=strokewidth;
...
function _colorMixin(options){
 ...
 return[s.fill,s.stroke,s.strokewidth];}else{
 return[
(o.fill !==undefined)?
...
function _ctx_stroke(stroke,strokewidth){
 ...
 _ctx.lineWidth=strokewidth;_ctx.stroke();}
}
...
var Path=BezierPath=Class.extend({
 ...
 _ctx_fill(a[0]);_ctx_stroke(a[1],a[2]);}else{
 _ctx.clip();}
 },
 angle: function(t){
 if(t==0){
...
var drawPath=drawpath;var autoClosePath=autoclosepath;var beginPath=beginpath;var moveTo=moveto;var lineTo=lineto;var curveTo=curveto;var closePath=closepath;var endPath=endpath;
...
function line(x0,y0,x1,y1,options){
 ...
 _ctx.beginPath();_ctx.moveTo(x0,y0);_ctx.lineTo(x1,y1);_ctx_stroke(a[1],a[2]);}
}
function rect(x,y,width,height,options){
 var a=_colorMixin(options);if(a[0]&& a[0].a > 0 || a[1]&& a[1].a > 0){
 if(!options || options.roundness===undefined){
 _ctx.beginPath();_ctx.rect(x,y,width,height);_ctx_fill(a[0]);_ctx_stroke(a[1],a[2]);}else{
 var p=new Path();p.rect(x,y,width,height,options);p.draw(options);}
 }
}
function triangle(x1,y1,x2,y2,x3,y3,options){
 var a=_colorMixin(options);if(a[0]&& a[0].a > 0 || a[1]&& a[1].a > 0){
 _ctx.beginPath();_ctx.moveTo(x1,y1);_ctx.lineTo(x2,y2);_ctx.lineTo(x3,y3);_ctx.closePath();_ctx_fill(a[0]);_ctx_stroke(a[1],a[2]);}
}
...
var Image=Class.extend({
 ...
 var o=options ||{};var w=(o.width !==undefined && o.width !=null)? o.width : this.width;var h=(o.height !==undefined && o.height !=null)? o.height : this.height;var a=(o.alpha !==undefined)? o.alpha : this.alpha;if(this._img.complete && w && h && a > 0){
 if(a >=1.0){
 _ctx.drawImage(this._img,x || this.x,y || this.y,w,h);}else{
 _ctx.globalAlpha=a;_ctx.drawImage(this._img,x || this.x,y || this.y,w,h);_ctx.globalAlpha=1.0;}
 }
 },
 busy: function(){
...
function absOffset(element){
 ...
 return[x,y];}
var DEFAULT="default";var HIDDEN="none";var CROSS="crosshair";var HAND="pointer";var POINTER="pointer";var TEXT="text";var WAIT="wait";
var Mouse=Class.extend({
 ...
 var m=this._mouse;m.pressed=false;m.dragged=false;m.drag.x=0;m.drag.y=0;if(!m._away()){
 m.onrelease(m);}
 ...
 m.ondrag(m);}else if(!m._away()){
 m.onmove(m);}
 };
 attachEvent(element,"mousedown",eventDown);attachEvent(element,"touchstart",eventDown);attachEvent(window,"mouseup",Function.closure(this.parent,eventUp));
...
function brightpass(img,threshold){
 var L=[0.2125/255,0.7154/255,0.072/255];return filter(img,function(p){
...
function blend(mode,img1,img2,dx,dy,alpha){
 ...
 case ADD : op=function(x,y){ return x+y;};break;case SUBTRACT : op=function(x,y){ return x+y-255;};break;case LIGHTEN : op=function(x,y){ return Math.max(x,y);};break;case DARKEN : op=function(x,y){ return Math.min(x,y);};break;case MULTIPLY : op=function(x,y){ return x*y/255;};break;case SCREEN : op=function(x,y){ return 255-(255-x)*(255-y)/255;};break;default : op=function(x,y){ return 0;};}
 var swap=(mode==LIGHTEN || mode==DARKEN || mode==MULTIPLY || mode==SCREEN);return composite(img1,img2,dx,dy,function(p1,p2){
 if(swap && p2[3]==255){
 var tmp=p1;p1=p2;p2=tmp;}
 var p=[0,0,0,0];var a=p2[3]/255*alpha;p[0]=geometry.lerp(p1[0],op(p1[0],p2[0]),a);p[1]=geometry.lerp(p1[1],op(p1[1],p2[1]),a);p[2]=geometry.lerp(p1[2],op(p1[2],p2[2]),a);p[3]=geometry.lerp(p1[3],255,a);return p;});}
